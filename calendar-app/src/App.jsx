...
import { CategoryContext, buildCategoryMap } from './CategoryContext';
...
function App() {
  ...
  const [sheetPassword, setSheetPassword] = useState(null);
  ...
  useEffect(() => {
    loadPassword().then((pw) => {
      setSheetPassword(pw);
      setPasswordLoaded(true);
      // If no password is set in the sheet, auto-authenticate
      if (!pw) {
  ...
  useEffect(() => {
    ...
    document.title = `CACF Calendar - All Months ${YEAR}`;
  ...
  // Show login page if password is set and user is not authenticated
  if (!authenticated && sheetPassword) {
        ...
        <div className="controls">
          ...
          <div className="controls-row">
            ...
            <button className="print-btn" onClick={() => {
              const prev = document.title;
              if (view === '1mo') {
                document.title = `CACF Calendar - ${MONTH_NAMES[startMonth]} ${YEAR}`;
              } else if (view === '2mo') {
                const months = [startMonth, startMonth + 1].filter(m => m < 12);
                document.title = `CACF Calendar - ${MONTH_NAMES[months[0]]}-${MONTH_NAMES[months[months.length - 1]]} ${YEAR}`;
              } else if (view === '3mo') {
                const months = [startMonth, startMonth + 1, startMonth + 2].filter(m => m < 12);
                document.title = `CACF Calendar - ${MONTH_NAMES[months[0]]}-${MONTH_NAMES[months[months.length - 1]]} ${YEAR}`;
              } else {
                document.title = `CACF Calendar - ${YEAR}`;
              }
              window.print();
              document.title = prev;
            }}>
